...
void posterizar(FILE *arq, FILE *arq_saida,matriz*m){
	...
	arq_saida = fopen("posterizarMedia.pgm","wb+");
	...
	for(i=0;i<m->height;i++){
		for(j=0;j<m->width;j++){
			...
			if(*(&m->data[i][j])<64)
				*(&m->data[i][j])=32;
			else if(*(&m->data[i][j])>63 && *(&m->data[i][j])<128)
				*(&m->data[i][j])=97;
			else if(*(&m->data[i][j])>127 && *(&m->data[i][j])<192)
				*(&m->data[i][j])=160;
			else if(*(&m->data[i][j])>191)
				*(&m->data[i][j])=224;
			}
		}
