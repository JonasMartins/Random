...
int media(matriz*m){
	...
	for(i=0;i<m->height;i++){
		for(j=0;j<m->width;j++){
			somatoria+=*(&m->data[i][j]);
		}
	}
	media=somatoria/(m->height*m->width);
	return media;
}


void localiza(FILE *arq, FILE *arq_saida,matriz*m){
